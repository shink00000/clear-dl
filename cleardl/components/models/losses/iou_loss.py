...
class MaskIoULossWithDistance(nn.Module):
    """ Mask IoU Loss (case: input and target are distance tensors)

    Args:
        input (torch.Tensor): [N, 36]
        target (torch.Tensor): [N, 36]

    Examples:
        >>> iou_loss = IouLoss(reduction='mean')
        >>> input = torch.rand(N, 36)
        >>> target = torch.rand(N, 36)
        >>> loss = lou_loss(input, target)
    ...
    def forward(self, input: torch.Tensor, target: torch.Tensor):
        inter = (torch.minimum(input, target) ** 2).sum(axis=1)
        union = (torch.maximum(input, target) ** 2).sum(axis=1)
        iou = inter / union
        loss = -iou.log()
